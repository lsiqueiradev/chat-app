  ...
  try {
    ...
    if (response.data) {
      ...
      if (!nextPage) {
        ...
        const messagesListFinal = messagesList.map((item) => {
          item._id = item.id;
          item.user._id = item.user_id;
          return item;
        });
        const joinArray = [...messagesListFinal, ...messages.reverse()];
        const mapa = new Map();
    ...
    Alert.alert('Erro', 'Confira seus dados1');
    yield put(getFailureDetails());
  }
}
export default all([
